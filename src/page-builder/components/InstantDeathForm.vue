  ...
  <div class="options goods-list-form">
    ...
    <el-form class="options__form" :model="formdata" size="small" label-position="left" label-width="80px">
      ...
      <el-form-item label="剩余库存">
        <div class="flex items-center w-full">
          <span class="el-form-item__value">{{ formdata.isShowStockNum ? '显示' : '不显示' }}</span>
          <el-switch
            class="ml-auto"
            v-model="formdata.isShowStockNum"
            @change="handleChange($event, 'isShowStockNum')"
          ></el-switch>
        </div>
      </el-form-item>
      <!-- 从商品里拿秒杀活动的时间 -->
...
import GoodsSelect from './select/InstantGoodSelect.vue';
